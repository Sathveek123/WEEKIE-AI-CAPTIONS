...
def generate_ass(
    ...
    *,
    ...
    for segment in (transcript.get("segments") or []):
        for word_info in (segment.get("words") or []):
            ...
            if (
                ...
                word_text = strip_emojis(str(w_raw).strip())
                if not word_text:
                    continue
    ...
    if is_latin_language(language):
        ...
        subs.info["WrapStyle"] = 1  # Smart wrap for Indic scripts
    ...
    if is_latin_language(language):
        ...
        new_style.fontname = get_fallback_font(language)
    ...
    for _, line_end, word_list in subtitles:
        for idx, (word, word_start, _, _) in enumerate(word_list):
            ...
            for i, (w, w_start, w_end, line_idx) in enumerate(word_list):
                ...
                w_display = w_str.upper() if is_latin_language(language) else w_str
